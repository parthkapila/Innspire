...
(P_HID IN CHAR(4), P_HFIRSTNAME IN VARCHAR2(15))
AS
    UPDATE HOST
...
(P_HID IN CHAR(4), P_HLASTNAME IN VARCHAR2(20))
AS
    UPDATE HOST
...
(P_HID IN CHAR(4), P_HEMAIL IN VARCHAR2(50))
AS
    UPDATE HOST
    SET EMAIL = P_HEMAIL
    WHERE HOSTID = P_HID;
...
(P_HID IN CHAR(4), P_HPHONE IN CHAR(12))
AS
    UPDATE HOST
    SET PHONE = P_HPHONE
    WHERE HOSTID = P_HID;
...
(P_CID IN CHAR(4), P_CFIRSTNAME IN VARCHAR2(15))
AS
    UPDATE CLIENT
    SET FIRSTNAME = P_CFIRSTNAME
    WHERE CLIENTID = P_CID;
    COMMIT;
EXCEPTION
    WHEN NO_DATA_FOUND THEN
...
(P_CID IN CHAR(4), P_CLASTNAME IN VARCHAR2(20))
AS
    UPDATE CLIENT
...
(P_CID IN CHAR(4), P_CEMAIL IN VARCHAR2(50))
AS
    UPDATE CLIENT
    SET EMAIL = P_CEMAIL
    WHERE CLIENTID = P_CID;
...
(P_CID IN CHAR(4), P_CPHONE IN CHAR(12))
AS
    UPDATE CLIENT
    SET PHONE = P_CPHONE
    WHERE CLIENTID = P_CID;
...
(P_CID IN CHAR(4), P_CADDR IN VARCHAR2(40))
AS
    UPDATE CLIENT
    SET ADDRESS = P_CADDR
    WHERE CLIENTID = P_CID;
...
(P_CID IN CHAR(4), P_CCITY IN VARCHAR2(20))
AS
    UPDATE CLIENT
    SET CITY = P_CCITY
    WHERE CLIENTID = P_CID;
...
(P_CID IN CHAR(4), P_CSTATE IN CHAR(2))
AS
    UPDATE CLIENT
    SET STATE = P_CSTATE
    WHERE CLIENTID = P_CID;
...
(P_LID IN CHAR(4), P_LPRICE IN NUMBER(7, 2))
AS
    UPDATE LISTING
...
(P_LID IN CHAR(4), P_LDESC IN VARCHAR2(1000))
AS
    UPDATE LISTING
...
(P_LID IN CHAR(4), P_LTITLE IN VARCHAR2(50))
AS
    UPDATE LISTING
    SET TITLE = P_LTITLE
    WHERE LISTINGID = P_LID;
...
(P_LID IN CHAR(4), P_DSTARTDATE IN DATE)
AS
...
(P_LID IN CHAR(4), P_DENDDATE IN DATE)
AS
    UPDATE DATES_AVAILABLE
...
(P_RID IN CHAR(4), P_RRATING IN NUMBER(2, 1))
AS
    UPDATE REVIEW
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE ('REVIEWID ' || P_LID || ' is not found.')
END;
/

CREATE OR REPLACE PROCEDURE UPDATE_REVIEW_DESC
(P_RID IN CHAR(4), P_RDESC IN VARCHAR2(500))
AS
    UPDATE REVIEW
...
(P_RID IN CHAR(4), P_RDATE IN DATE)
AS
    UPDATE REVIEW
